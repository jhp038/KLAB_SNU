function cell_registered_struct=CellReg_KLAB(neurons)
%% Method overview:
...
%for KLAB's purpose, we have to change the format to 
% 1. change directory via .. and get pwd
cd ..
current_directory = pwd;
% Defining the results_directory and creating the figures_directory:
results_directory='current_directory';
figures_directory=fullfile(results_directory,'Co-registration Figures');
...
%% now, we are going to create .mat spatial files and keep their file names. 
totalNumNeuron = size(neurons,1);
spatial_footprints = [];
for neuronNum = 1: totalNumNeuron
    %extract and save
    neuron = neurons{neuronNum};
    number_of_cells = size(neuron.A , 2);
    [d1 d2] = size(neuron.Cn);
    for n=1:number_of_cells
        spatial_footprints{1,neuronNum}(n,:,:) = reshape(neuron.A(:,n), d1 , d2);
    end
end
...
microns_per_pixel=1; %preset = 2.4
...
[all_projections_correlations,number_of_cells_per_session]=...
    evaluate_data_quality(spatial_footprints_corrected,centroid_projections_corrected,footprints_projections_corrected,maximal_cross_correlation,alignment_translations,reference_session_index,sufficient_correlation,alignment_type);
