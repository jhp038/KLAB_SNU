...
for mouseNum = 1:totalMouseNum
    ...
    disp(['Loading noldus data from : ' num2str(filePathList{mouseNum})]);
    [currentData, text, ~]  = xlsread([pwd filesep fpObj.idvData(mouseNum).folder filesep num2str(filePathList{mouseNum})]);
    idx = find(strcmp(text,'Trial time'));
    fpObj.idvData(mouseNum).noldusHeader = text(idx,:);
    %modified Raw Data's idx. Use LEDOn Idx to crop data.
    noldusRawData = currentData(LEDOnIdx(1):min(length(currentData),LEDOnIdx(end)),:);
%     noldusRawData(:,1) = noldusRawData(:,1) - noldusRawData(1,1);
    fpObj.idvData(mouseNum).noldusRawData = currentData(LEDOnIdx(1):min(length(currentData),LEDOnIdx(end)),:);
