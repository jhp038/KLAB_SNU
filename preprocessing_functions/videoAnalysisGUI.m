...
% Revised by Jong Hwi Park for behavioral tracking purposes @ K-LAB SNU 

% version 1.05
function [boutArray nOfFrames] = videoAnalysisGUI(pathToSave)
...
current_patch= patch([0 0 0 0], [0.5 0.5 1 1], 'b', 'Parent', axHandle, 'EdgeColor','k','ButtonDownFcn',@locMouseButtonDownFcnSearchBar);
blue_patch = patch([0 0 0 0], [0.5 0.5 1 1], 'b', 'Parent', axHandle, 'EdgeColor','k','ButtonDownFcn',@locMouseButtonDownFcnSearchBar);
...
while ishandle(hFig) && ~bKillAll
    ...
    %added by JH to visualize bout on searchbar  
    if mod(fr,10) == 0
    disp([boutArray])
    end
    
    if length(boutArray) >= 2 &&  mod(length(boutArray),2) == 0  %if boutArray has more than 2 element and EVEN
        %drawing all the patch
        boutArray = sort(boutArray);
        boutArray_resized = vec2mat(boutArray,2);
        boutArray_resized = repmat(boutArray_resized./macroDat.nofFrames,1,2);
        boutArray_resized(:,[3,4]) = boutArray_resized(:,[4,3]);
        boutArray_resized = boutArray_resized';
        num = size(boutArray_resized,2);
        y_array = repmat([0;0;1;1],1,num);
        set(blue_patch,'XData',boutArray_resized,'YData',y_array,'FaceAlpha',0.3);
        set(current_patch,'XData',[0 0 0 0]);
        
    elseif mod(length(boutArray),2) == 1 && boutArray(i-1,1) < fr && length(boutArray) >2  %if boutArray has odd num of element and previous chosen frame is smaller than current frame AND has more than 2 element
        %draw even patch
        boutArray_temp = sort(boutArray(1:end-1));
        boutArray_resized = vec2mat(boutArray_temp,2);
        boutArray_resized = repmat(boutArray_resized./macroDat.nofFrames,1,2);
        boutArray_resized(:,[3,4]) = boutArray_resized(:,[4,3]);
        boutArray_resized = boutArray_resized';
        num = size(boutArray_resized,2);
        ...
        curBout = boutArray(end)./macroDat.nofFrames;
        curFrame = fr/macroDat.nofFrames;
        %drawing progressing patch
        seekProgress = fr/macroDat.nofFrames;
        set(current_patch, 'XData',[curBout seekProgress seekProgress curBout])
        
    elseif mod(length(boutArray),2) == 1 && boutArray(i-1,1) < fr && length(boutArray) == 1%if boutArray has only 1 element
        curBout = boutArray(end)./macroDat.nofFrames;
        curFrame = fr/macroDat.nofFrames;       
        %drawing progressing patch
        seekProgress = fr/macroDat.nofFrames;
    ...
    elseif isempty(boutArray)
        set(blue_patch,'XData',[0 0 0 0])
    end
...
%reshape output boutArray
boutArray = reshape(boutArray,2,[])';
...
colIm = hsv2rgb(H,  ones(size(H),'single'),V);
    ...
    function locKeypress(~,evnt)
        switch evnt.Key,
            ...
            case 'z'
                if i > 1&& mod(length(boutArray),2) == 1 && boutArray(i-1,1) >fr
                    bSeeking = 0;
                    bPause = 0;
                    disp('CANT DO THAT')
                else
            ...
            case 'delete'
                for z = 1:2:length(boutArray)-1
                    if fr >= boutArray(z) && fr <=boutArray(z+1)
                        disp(['index is: ' num2str(z)])
                        boutArray(z:z+1) = [];
                        i = i - 2;
                        bPause = 0;
                        bSeeking = 1;
