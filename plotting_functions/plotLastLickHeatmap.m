function plotLastLickHeatmap(fpObj,upperExamRange)
...
for fpObjNum = 1:totalfpObjNum
    for mouseNum = 1:totalMouseNum
        ...
        plotRangeIdx = boutIdx(:,2) + examRangeIdx;
        ...
        hold on
        plot([0,0],ylim, 'LineStyle', '--', 'Color', [1 1 1], 'LineWidth', 2);
        titleString = [fpObj(fpObjNum).idvData(mouseNum).Description ' \_Raw \DeltaF/F LastLick'];
        title(titleString,'FontSize',10);
        saveas(gcf,[fpObj.idvData(mouseNum).Description ' Last Lick heatmap' '.jpg'])        
        ...
        hold on
        plot([0,0],ylim, 'LineStyle', '--', 'Color', [1 1 1], 'LineWidth', 2);
        titleString = [fpObj(fpObjNum).idvData(mouseNum).Description ' \_Raw \DeltaF/F mean lastLick'];
        title(titleString,'FontSize',10);
        saveas(gcf,[fpObj.idvData(mouseNum).Description ' last Lick mean heatmap' '.jpg'])       
    end
end