...
alreadyRanGetPupilROI = 1; %%% default should be 0
subvideoAlreadyLoaded = 1; %%% default should be 0
...
subVidName_minusNumber = 'C:\Users\User\Documents\GitHub\KLAB_SNU\pupil_diameter_functions\pupillometry-master\oldVersionsRat1\tsaoPupil.mat'; % MUST BE .mat format!!! video directory and file name (without number and extension)
%%% END PARAMETERS
...
%         imshow(frame(roi_topLeftY:roi_bottomLeftY,roi_topLeftX:roi_topRightX),'DisplayRange', [184 4092])
        imshow(frame(roi_topLeftY:roi_bottomLeftY,roi_topLeftX:roi_topRightX))
...
%     frame = rgb2gray(frame);
    ...
    START_FRAME = 1000; %%%%%%%%%%%% frame to start analysis at
...
%         frame = mov(:,:,frameNum); % DORIS STYLE MOVIES
...
%         frame = imfilter(frame,filter); % filter defined in the parameters above
        ...
        measurePupil_04_07_17; %%%%%%%%% script handling actual pupil measurement (along with saving and plotting (if desired))
