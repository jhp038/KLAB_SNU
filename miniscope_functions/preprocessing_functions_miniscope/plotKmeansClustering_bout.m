function plotKmeansClustering_bout(msObj,examRange,k,iteration)
...
for neuronNum = 1: numTotalNeuron
    ...
    %first neuron's data
    idx_last = [videoEventIdx(:,2) videoEventIdx(:,2)] + repmat(examRangeIdx,[numEvent 1]);
    for i = 1:numEvent%heating num
        if idx(i,2) < numOfFrames
            df_neuron(i,:) = data_norm(neuronNum,idx(i,1):idx(i,2));
        end
        if idx_last(i,2) <numOfFrames
            df_neuron_last(i,:) = data_norm(neuronNum,idx_last(i,1):idx_last(i,2));
        end
    end
    mean_norm_array(neuronNum,:) = mean(df_neuron);
    mean_norm_lastarray(neuronNum,:) = mean(df_neuron_last);
...
mean_norm_lastarray = mean_norm_lastarray-repmat(baselineMean,[1,examRangeSize]);

%gotta get size of array
arraySize = size(mean_norm_array);


%% kmeans firstlick
totalNumK =k;
temparray = [mean_norm_array mean_norm_lastarray];
...
for i = 1:2
    if i ==1
        ...
        plottingArray = temparray(:,1:arraySize(2));
    else
        f1 = figure('Units','inch','Position',[5 1 4 8],'Visible','on');
                
        plottingArray =temparray(:,arraySize(2)+1:end);
    end
    imagesc( timeV, 1:size(idx,1), plottingArray)
    hold on
